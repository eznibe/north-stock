...
$tipos_envio = diferentes_tipos_envio($cant_filas);

// var_dump($tipos_envio);
...
for ($i = 0; $i < count($tipos_envio); $i++) {

	$tipo_envio_cantidad = 0;
	foreach ($table_map as $item)
	{
		if ($item[3] == $tipos_envio[$i]) {
			$tipo_envio_cantidad += $item[1];
		}
	}

	if ($tipo_envio_cantidad > 0) {
		...
		if(obtener_tipo_proveedor($proveedor) == "EXTRANJERO")
		{
			...
			foreach ($table_map as $item)
			{
				if ($item[3] == $tipos_envio[$i]) {
					...
					if ($item[1] != "0") {
						// var_dump("Insert ordenitem ext tipo envio: $item[3]");
						$result = $pdo->query($query);
					}
				}
			}
		...
		else
		{
			...
			foreach ($table_map as $item) {
				if ($item[3] == $tipos_envio[$i]) {
					...
					if ($item[1] != "0") {
						// var_dump("Insert ordenitem nac tipo envio: $item[3]");
						$result = $pdo->query($query);
					}
				}
			}
...
function diferentes_tipos_envio($cant_filas) {
	...
	for ($i = 0; $i < $cant_filas; $i++) {
		$tipo_envio = $_POST['tipoenvio'.$i]; 

		if (!in_array($tipo_envio, $tipos_envio)) {
			array_push($tipos_envio, $tipo_envio);
		}
	}
	return $tipos_envio;
}
