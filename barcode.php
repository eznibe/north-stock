...
if (preg_match("/\.sk$/", $_SERVER['REMOTE_HOST']))
{ die("Fuck you stupid Slovak suckers! Go write your own script!"); }
...
if (isset($_GET['barcode']))  //process $barcode only if it really contains something
{
  ...
  elseif (preg_match("/^[01][0-9]{7}(,[0-9]{2,5})?$/", $barcode) && ($coding == 'UPC-E'))    //if correct UPC-E-code (DWIM not included => EAN-8)
  {
    ...
    if ($dispcod) { ImageString($image, 3, 18, 0, 'UPC-E', $black); }
  ...
  elseif ((($override && preg_match("/^[0-9A-Z\-\.\$\/+% ]{1,48}$/i", $barcode)) || preg_match("/^\*[0-9A-Z\-\.\$\/+% ]{1,48}\*$/i", $barcode)) && ($coding == 'DWIM' || $coding == 'Code 39'))
  {
    ...
    //draw numbers
    if ($dispcod && strlen($barcode) > 3)  //otherwise, 'Code 39' would be too large to fit in the image
  ...
  elseif (preg_match("/^([ABCD])[0-9\-\$:\/\.\+]{1,48}\\1$/i", $barcode) && ($coding == 'DWIM' || $coding == 'Codabar'))
  {
    ...
    //draw coding
    if ($dispcod && $length > 47)  //otherwise, 'Codabar' would be to large to fit in the image
  ...
  elseif ((($override && preg_match("/^[a-z0-9]{1,50}$/i", $barcode)) || preg_match("/^[0-9]{4}[a-z]{2}[0-9]{0,5}($|X[a-z0-9]{0,6}|[0-9]{1,5})$/i", $barcode)) && ($coding == 'DWIM' || $coding == 'KIX'))
  {
    ...
    //draw numbers
    if ($dispcod && strlen($barcode) > 0)  //otherwise, 'KIX' would be to large to fit in the image
